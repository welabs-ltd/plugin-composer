	...
	useEffect(() => {
        ...
        const fetchSettings = async () => {
            try {
				...
				if( response.msf_page_title ){
					setPageTitle(response.msf_page_title);
				}
	...
	useEffect(() => {
        ...
        const fetchPages = async () => {
            try {
				...
				options.unshift({ value: '', label: __( 'Select dashboard page', 'welabs-settings' ), disabled: true });
		...
		try {
			const { msf_dashboard_page_id } = await apiFetch( {
				...
				data: {
					...
					msf_page_title: pageTitle,
				},
			} );
			setDashboardPage( msf_dashboard_page_id );
			setMessage(
				__( 'Settings saved successfully!', 'welabs-settings' )
			);
			setError( '' );
			setIsLoading( false );
		} catch (error) {
	...
	return (
		<div>
			<div className='settings-header'>
				...
				<h2>{ __( 'General Settings', 'welabs-settings' ) }</h2>
			</div>
			{ message && (
				<Notice
			...
			<form onSubmit={ handleSubmit }>
				<Card>
					<CardBody>
						...
						/>
						<TextControl
							label={ __( 'Page Title', 'welabs-settings' ) }
							value={ pageTitle }
							onChange={ setPageTitle }
						/>
						<Button variant="primary" type="submit" disabled={ isLoading }>
							{ isLoading && <Spinner /> }
							{ __( 'Save Changes', 'welabs-settings' ) }
						</Button>
					</CardBody>
				</Card>
			</form>
